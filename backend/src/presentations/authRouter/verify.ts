...
import { LoginAccount, loginAccountValidator } from './accountValidator';

export function makeVerify(db: Database) {
  return (
    id: string,
    password: string,
    callback: (error: Error | null, user?: LoginAccount | false, options?: IVerifyOptions) => void
  ) => {
    ...
    try {
      ...
      const account = loginAccountValidator.parse({ id: row.loginId, password: row.password });
